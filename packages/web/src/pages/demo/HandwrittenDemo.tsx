...
import wasmUrl from "num-wasm/dist/num-wasm.wasm?url";
...
export default function App() {
  ...
  const [theme, setTheme] = useState<"system" | "light" | "dark">(
    () => (typeof localStorage !== "undefined" ? (localStorage.getItem("numwasm-theme") as "system" | "light" | "dark") || "light" : "light"),
  );
  ...
  useEffect(() => {
    localStorage.setItem("numwasm-theme", theme);
  ...
  const loadModelFile = async (file: File) => {
    ...
    try {
      ...
      setInitError("invalid model file");
    }
  };
  ...
  return (
    <div className="page">
      <header className="masthead">
        ...
        <p className="kicker">
          <span className="brand-dim">@felixfern</span>/num-wasm/handwritten
        </p>
        <p className="lede">
          MLP trained in the browser on WebAssembly — draw a digit, the net
          reads it back.
        </p>
        <button className="theme-btn" title={`theme: ${theme}`} onClick={cycleTheme} aria-label="toggle theme">
